...
/*
  ...
  Riempie il proprio contenitore. I colori fanno MORPH (lerp) quando cambia la prop
  → sfondo persistente che cambia palette per schermo senza rimontare il context.
*/
...
const fragment = /* glsl */ `
  ...
  void main(){
    ...
    float w1 = 1.0 / (0.035 + dot(p - b1, p - b1));
    float w2 = 1.0 / (0.035 + dot(p - b2, p - b2));
    float w3 = 1.0 / (0.035 + dot(p - b3, p - b3));
    float w4 = 1.0 / (0.035 + dot(p - b4, p - b4));
    float ws = w1 + w2 + w3 + w4;
    vec3 col = (uC1 * w1 + uC2 * w2 + uC3 * w3 + uC4 * w4) / ws;
    col += 0.035 * sin((vUv.x + vUv.y) * 40.0 + uTime);
...
export function MeshGradient({ colors = DEFAULT, className, speed = 1 }: MeshGradientProps) {
  ...
  useEffect(() => {
    ...
    const [c1, c2, c3, c4] = colorsRef.current.map(hexVec);
    ...
    const lerp = (u: number[], target: [number, number, number], k: number) => {
      u[0] += (target[0] - u[0]) * k;
      u[1] += (target[1] - u[1]) * k;
      u[2] += (target[2] - u[2]) * k;
    };
    ...
    const loop = (ms: number) => {
      ...
      const tgt = colorsRef.current.map(hexVec) as [number, number, number][];
      const k = 0.04;
      lerp(program.uniforms.uC1.value as number[], tgt[0], k);
      lerp(program.uniforms.uC2.value as number[], tgt[1], k);
      lerp(program.uniforms.uC3.value as number[], tgt[2], k);
      lerp(program.uniforms.uC4.value as number[], tgt[3], k);
